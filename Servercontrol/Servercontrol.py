from flask import Flask,render_template,request,redirect,g,session,url_for # import flask for the web application deployment function 
...
Listdict = [5,6]
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # Authentication for the Server user stafff only access
class User:
...
users = []
users.append(User(id=1, username='Idatabots', password='Rkj3548123#'))
